// FRONTEND/src/pages/Educator/EditCourse.jsx
...
function EditCourse() {
  ...
  const handleEditCourse = async () => {
    ...
    try {
      // FIX: Correctly pass formData as the body and withCredentials in the config
      const result = await axios.post(
        `${serverUrl}/api/course/editcourse/${courseId}`,
        formData, // The data payload
        { withCredentials: true } // The configuration object
      );
      console.log(result.data);
      setLoading(false);
      navigate("/courses");
  ...
  return (
    <div className='max-w-5xl mx-auto p-6 mt-10 bg-white rounded-lg shadow-md'>
      <div className='flex items-center justify-center gap-[20px] md:justify-between flex-col md:flex-row mb-6 relative '>
        ...
        <h2 className='text-2xl font-semibold pl-[30px] md:pl-[60px]'>
      ...
      <div className='bg-gray-50 p-6 rounded-md'>
        ...
        <div className='space-x-2 space-y-2 '>
          {!isPublished ? (
            <button className='bg-green-100 text-green-600 px-4 py-2 rounded-md border-1' onClick={() => setIsPublished(prev => !prev)}>Click to Publish</button>
          ) : (
        ...
        <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); }}>
          ...
          <div className='relative w-[300px] h-[170px] mt-4'>
          ...
          <div className='flex items-center justify-start gap-[15px] mt-4'>
            ...
            <button className='bg-black text-white px-7 py-2 rounded-md hover:bg-gray-500 cursor-pointer' onClick={handleEditCourse} disabled={loading}>{loading ? <ClipLoader size={30} color={"#fff"} /> : "Save"}</button>
          </div>
        </form>
      </div>
    </div>
